function [ P ] = solve_stator(gl, P0, triangle, Yp, beta)%, k1, cp)
    
    P = init_point();
    
    P.hT = P0.hT; % fix
    P.v = triangle.v1;
    P.h = P.hT - P.v^2 / 2; % fix
    
    % first guess of p
    P.p = P0.p / (sqrt(beta));
    
    err = inf;
    old = gl.pT0;
    ii = 0;
    
    p1 = P.p;
    
    while err > gl.toll
        
        P.rho = XSteam('rho_ph', p1, P.h / 1000);
        
        % definition of pT1 = @(p1) p1 + P.rho * P.v^2 / 2;
        % Yp  = (pT0 - pT1) / (pT1 - p1);
        p1 = P0.pT - (1 + Yp) * P.rho * P.v^2 / 2 / 1e5;
        
        err = abs(p1 - old);
        old = p1;
        
        ii = ii + 1;
    end
    
    P.p = p1;
    P.rho = XSteam('rho_ph', P.p, P.h / 1000);
    P.mu = XSteam('my_ph', P.p, P.h / 1000);
    P.T = XSteam('T_ph', P.p, P.h / 1000) + 273.15;
    P.s = XSteam('s_ph', P.p, P.h / 1000);
    P.cp = XSteam('Cp_pT', P.p, P.T - 273.15) * 1000;
    
    P.TT = P.T + P.v^2 / 2 / P.cp;
    P.pT = P.p + P.rho * P.v^2 / 2 / 1e5;
    P.hTrel = P.h + triangle.w1^2 / 2;
    P.TTrel = P.T + triangle.w1^2 / 2 / P.cp;
    P.pTrel =  P.p + P.rho * triangle.w1^2 / 2 / 1e5;
    P.sT = XSteam('s_pT', P.pT, P.TT - 273.15);
  
    %P.v = triangle.v1;
    P.a = XSteam('w_pT', P.p, P.T - 273.15);
    P.M = triangle.v1 / P.a;
    P.Mrel = triangle.w1 / P.a;
    
    % as gamma we use a mean value between inlet and outlet
    P.hIS = XSteam('h_ps', P.p, P0.s) * 1000;
end
