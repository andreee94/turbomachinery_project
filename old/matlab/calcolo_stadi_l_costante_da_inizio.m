function [ ST,N,n_intermedio ] = calcolo_stadi_l_costante_da_inizio(N,ST0)
...
for ii = 1:N
    ...
    ST(ii).bsuD = m_P/(ST(ii).rho_IN*v_M*ST(ii).D^2*pi);
    ST(ii).Mach = ST(ii).U / sqrt(gamma*R*ST(ii).T_IN);
